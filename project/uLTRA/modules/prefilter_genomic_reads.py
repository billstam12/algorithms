...
def align_with_minimap2(refs_path, read_path, outfolder, nr_cores, k_size, minimap_path):
    ...
    with open(minimap2_samfile_path, "w") as output_file:
        ...
        subprocess.check_call([ minimap_path,  '-ax', 'splice', '--eqx' , '-k', str(k_size), '-t', str(nr_cores), refs_path, read_path], stdout=output_file, stderr=stderr_file)
        # print('Done.')
        sys.stdout.flush()
    output_file.close()
...
def main(ref_part_sequences, ref, reads, outfolder, indexfolder, nr_cores, genomic_frac_cutoff, k_size, minimap_path):
    indexed_regions = get_ultra_indexed_choordinates(ref_part_sequences, indexfolder, outfolder)
    minimap2_samfile_path = align_with_minimap2(ref, reads, outfolder, nr_cores, k_size, minimap_path)
